#!/usr/bin/env python3
"""Offline tests for fetch_pollen.py — no network. Run: python3 test_fetch_pollen.py"""
...
def test_category_for():
    assert fetch_pollen.category_for(28, 82) == "high"       # >= 15
    assert fetch_pollen.category_for(28, 10) == "moderate"   # 5 <= x < 15
    assert fetch_pollen.category_for(28, 3) == "low"         # < 5
    assert fetch_pollen.category_for(28, -1) is None         # no data
    assert fetch_pollen.category_for(28, None) is None
    assert fetch_pollen.category_for("99", 5) is None        # unknown id


def test_parse_feed():
    date, pollen_map = fetch_pollen.parse_feed(_sample_raw())
    ...
    assert set(pollen_map) == set(fetch_pollen.POLLEN_NAMES.values())


def test_parse_feed_missing_station():
    raw = json.dumps(json.dumps({"fields": {"49": {}}}))
    try:
        fetch_pollen.parse_feed(raw)
...
def test_last_measurement_date_and_skip():
    with tempfile.TemporaryDirectory() as d:
        ...
        assert fetch_pollen.last_measurement_date(path) is None  # missing file
        with open(path, "w", encoding="utf-8") as f:
            f.write(json.dumps({"measurement_date": "2026-06-20"}) + "\n")
            f.write(json.dumps({"measurement_date": "2026-06-22"}) + "\n")
        assert fetch_pollen.last_measurement_date(path) == "2026-06-22"
...
def test_skip_logic_does_not_append(monkeypatched_feed_date="2026-06-22"):
    with tempfile.TemporaryDirectory() as d:
        ...
        previous = fetch_pollen.last_measurement_date(path)
        new_date = "2026-06-22"
